...
class TradeModel extends \Think\Model
{
	...
	public function moni($market = NULL)
	{
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from btchanges_trade where userid=' . userid() . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables btchanges_trade write ,btchanges_user_coin write,btchanges_finance write');
		$rs = array();
		...
		if ($type == 1) {
			...
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $userid))->setDec($rmb, $mum);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $userid))->setInc($rmb . 'd', $mum);
			$rs[] = $finance_nameid = $mo->table('btchanges_trade')->add(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
			...
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $userid))->setDec($xnb, $num);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $userid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('btchanges_trade')->add(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
	...
	public function dapan($market = NULL)
	{
		...
		for (; $i < 30; $i++) {
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables btchanges_trade write ,btchanges_trade_log write ,btchanges_user write, btchanges_user_coin write,btchanges_invit write ,btchanges_finance write');
				...
				$rs[] = $finance_nameid = $mo->table('btchanges_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $amount);
				$finance = $mo->table('btchanges_finance')->where(array('userid' => $buy['userid']))->order('id desc')->find();
				$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->find();
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $save_buy_rmb);
				$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->find();
				...
				$finance = $mo->table('btchanges_finance')->where(array('userid' => $sell['userid']))->order('id desc')->find();
				$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->find();
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->setInc($rmb, $sell_save);
				$save_buy_rmb = $sell_save;
				$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->find();
						...
						if ($buy_fee) {
							if ($invit_buy_user['invit_1']) {
								...
								if (0 < $invit_buy_save_1) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_buy_user['invit_1']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_1']))->find();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_1']))->find();
							...
							if ($invit_buy_user['invit_2']) {
								...
								if (0 < $invit_buy_save_2) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_buy_user['invit_2']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->find();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->find();
							...
							if ($invit_buy_user['invit_3']) {
								...
								if (0 < $invit_buy_save_3) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_buy_user['invit_3']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->find();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->find();
						...
						if ($sell_fee) {
							if ($invit_sell_user['invit_1']) {
								...
								if (0 < $invit_sell_save_1) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_sell_user['invit_1']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->find();
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->setInc($rmb, $invit_sell_save_1);
									$rs[] = $mo->table('btchanges_user_coin')->getLastSql();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->find();
							...
							if ($invit_sell_user['invit_2']) {
								...
								if (0 < $invit_sell_save_2) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_sell_user['invit_2']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->find();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->find();
							...
							if ($invit_sell_user['invit_3']) {
								...
								if (0 < $invit_sell_save_3) {
									$finance = $mo->table('btchanges_finance')->where(array('userid' => $invit_sell_user['invit_3']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->find();
									...
									$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->find();
	...
	public function chexiao($id = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables btchanges_user_coin write  , btchanges_trade write ,btchanges_finance write');
		$rs = array();
		$user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();
		...
		if ($trade['type'] == 1) {
			...
			$finance = $mo->table('btchanges_finance')->where(array('userid' => $trade['userid']))->order('id desc')->find();
			$finance_num_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$finance_mum_user_coin = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			if (!$you_sell) {
				...
				if (0 < $you_user_sell[$xnb . 'd']) {
					$mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setField($xnb . 'd', 0);
				}
			}
		}
		else {
