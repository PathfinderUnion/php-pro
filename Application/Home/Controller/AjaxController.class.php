...
class AjaxController extends HomeController
{
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if (!$data_getDepth[$market][$trade_moshi]) {
			...
			if ($trade_moshi == 1) {
				$buy = $mo->query('select id,price,sum(num-deal)as nums from btchanges_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit ' . $limt . ';');
				$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from btchanges_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit ' . $limt . ';'));
			}
