...
class IssueController extends HomeController
{
	...
	public function upbuy($id, $num, $paypassword)
	{
		...
		$mo->execute('lock tables btchanges_invit write ,  btchanges_user_coin write  , btchanges_issue write  , btchanges_issue_log  write ,btchanges_finance write');
		$rs = array();
		...
		$rs[] = $finance_nameid = $mo->table('btchanges_issue_log')->add(array('userid' => userid(), 'coinname' => $Issue['coinname'], 'buycoin' => $Issue['buycoin'], 'name' => $Issue['name'], 'price' => $Issue['price'], 'num' => $num, 'mum' => $mum, 'ci' => $Issue['ci'], 'jian' => $Issue['jian'], 'unlock' => 1, 'addtime' => time(), 'endtime' => time(), 'status' => $Issue['ci'] == 1 ? 1 : 0));
