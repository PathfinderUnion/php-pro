...
class FinanceController extends HomeController
{
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
		$rs[] = $finance_nameid = $mo->table('btchanges_mytx')->add(array('userid' => userid(), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function mytxChexiao($id)
	{
		...
		$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
	...
	public function upmyzc($coin, $num, $addr, $paypassword, $moble_verify)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			...
			$rs[] = $mo->table('btchanges_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('btchanges_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$mo->execute('lock tables  btchanges_user_coin write  , btchanges_myzc write ,btchanges_myzr write, btchanges_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('btchanges_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $aid = $mo->table('btchanges_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));
