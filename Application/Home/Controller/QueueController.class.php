...
// 	}
	...
	public function checkYichang()
	{
		...
		$mo->execute('lock tables btchanges_trade write');
		$Trade = M('Trade')->where('deal > num')->order('id desc')->find();

		if ($Trade) {
			if ($Trade['status'] == 0) {
				$mo->table('btchanges_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num']), 'status' => 1));
			}
			else {
				$mo->table('btchanges_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num'])));
			}
		...
		foreach ($coinList as $k => $v) {
			...
			foreach ($listtransactions as $trans) {
				...
				if ($trans['category'] == 'receive') {
					...
					$rs = array();
					$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $user['id']))->setInc($coin, $trans['amount']);
					...
					if ($res = $mo->table('btchanges_myzr')->where(array('txid' => $trans['txid']))->find()) {
						...
						$rs[] = $mo->table('btchanges_myzr')->add(array('userid' => $user['id'], 'username' => $trans['address'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['txid'], 'num' => $true_amount, 'mum' => $trans['amount'], 'addtime' => time(), 'status' => 1));
					}
