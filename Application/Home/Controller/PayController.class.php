...
class PayController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			if (isset($_POST['alipay'])) {
				...
				$mo->execute('lock tables btchanges_user_coin write,btchanges_mycz write,btchanges_finance write');
				$rs = array();
				...
				$rs[] = $mo->table('btchanges_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function movepay()
	{
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables btchanges_user_coin write,btchanges_mycz write');
			$rs = array();
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = $mo->table('btchanges_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
